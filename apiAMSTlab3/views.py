...
from apiAMSTlab3.models import Sensores,Lecturas
from apiAMSTlab3.serializer import SensoresSerializer,LecturasSerializer
...
def lectura_data_detail(request, pk):
    try:
        ...
        if request.method == 'GET':
            ...
            lectura_data.delete()
            return JsonResponse({'message': 'sensor data was deletedsuccessfully!'},status=status.HTTP_204_NO_CONTENT)
    except Sensores.DoesNotExist:
        return JsonResponse({'message': 'The sensor data does not exist'},status=status.HTTP_404_NOT_FOUND)
